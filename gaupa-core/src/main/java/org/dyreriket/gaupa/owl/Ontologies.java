package org.dyreriket.gaupa.owl;
...
public class Ontologies {
    ...
    /*
     * public static OWLOntology getEmptyOntology () { try { return
     * OWLManager.createOWLOntologyManager().createOntology(); } catch
     * (OWLOntologyCreationException e) {
     * System.err.println("Error creating empty ontology: "); e.printStackTrace(); }
     * return null; }
     */
     ...
     * Model model) {
     *
     * OWLRDFConsumer consumer = new OWLRDFConsumer(ontology, new
     * OWLOntologyLoaderConfiguration());
     *
     * for (Triple t : Models.toTripleSet(model)) { Node subject = t.getSubject();
     * Node predicate = t.getPredicate(); Node object = t.getObject(); if
     * (object.isLiteral()) { String oValue = object.toString(); String oDatatype =
     * object.getLiteralDatatypeURI(); String oLanguage =
     * object.getLiteralLanguage();
     * consumer.statementWithLiteralValue(getURI(subject), getURI(predicate),
     * oValue, oLanguage, oDatatype); } else {
     * consumer.statementWithResourceValue(getURI(subject), getURI(predicate),
     * getURI(object)); } } }
     *
     * private static String getURI (Node resourceNode) { return
