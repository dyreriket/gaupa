...
public class RDFLists {
    ...
    /**
     * This is a almost verbatim copy of Jena's RDFlistImpl.copy method, but there
     * the list items are not typed as lists as the native list syntax (a b c) fails
     * to kick in. Note that the copy is performed inplace.
     *
     * @param inlist
     * @return
     */
    ...
    /**
     * Selects only rdf:first and rdf:rest statements that make up the list,
     * recursively including lists in the list.
     *
    ...
    /*
     * public static List<Statement> copyList (RDFList head) { List<Statement> copy
     * = new ArrayList<>();
     *
     * Resource first = head; Resource rest; for (ExtendedIterator<RDFNode> it =
     * head.iterator(); it.hasNext(); ){ // first, the content element RDFNode
     * content = it.next(); copy.add(ResourceFactory.createStatement(first,
     * RDF.first, content)); // recursive call, if content is a list if
     * (content.canAs(RDFList.class)) {
     * copy.addAll(copyList(content.as(RDFList.class))); } // rest: either more
     * lists elements, or nil if (it.hasNext()) { rest =
     * ResourceFactory.createResource(); } else { rest = RDF.nil; }
     * copy.add(ResourceFactory.createStatement(first, RDF.rest, rest)); // prepare
     * for next list element first = rest; } return copy; }
     */
